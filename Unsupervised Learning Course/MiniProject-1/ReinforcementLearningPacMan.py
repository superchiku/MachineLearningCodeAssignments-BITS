...
def ppo_loss(oldpolicy_probs, advantages, rewards, values):
    ...
    def loss(y_true, y_pred):
        ...
        y_old = oldpolicy_probs
        newpolicy_probs = K.sum(y_true * y_pred, axis=1)
        y_old = K.sum(y_true * y_old, axis=1)
        #ratio = K.exp(K.log(newpolicy_probs + 1e-10) - K.log(y_old + 1e-10))
        #ratio = K.exp(K.log(newpolicy_probs) - K.log(y_old + 1e-10))
        ratio = newpolicy_probs/(y_old + 1e-10)
        p1 = ratio * advantages
        p2 = K.clip(ratio, min_value=1 - clipping_val, max_value=1 + clipping_val) * advantages
        actor_loss = -K.mean(K.minimum(p1, p2))

        total_loss = actor_loss + -K.mean(entropy_beta*
...
class PPO:
    ...
    #commenting out original imagenet based cnn filteration
    '''
    ...
    def get_model_actor_image(self,input_dims, output_dims):
        ...
        # Classification block
        '''
        conv_1 = layers.Conv2D(filters=32, kernel_size=(8, 8), strides=(4, 4), activation='relu', padding="same")(
            layers.Lambda(lambda x: x / 255.0)(state_input))
        '''
        conv_1 = layers.Conv2D(filters=32, kernel_size=(8, 8), strides=(4, 4), activation='relu', padding="same")(
            state_input)
        conv_2 = layers.Conv2D(filters=64, kernel_size=(4, 4), strides=(2, 2), activation='relu', padding="same")(
            conv_1)
        conv_3 = layers.Conv2D(filters=64, kernel_size=(3, 3), strides=(1, 1), activation='relu', padding="same")(
            conv_2)
        conv_flattened = layers.Flatten(name='flatten')(conv_3)
        x = Dense(512, activation='relu', name='fc1')(conv_flattened)
        out_actions = Dense(output_dims, activation='softmax', name='predictions')(x)
    ...
    def test_reward(self):
        ...
        cur_state = np.stack(self.seq_memory, axis=2)
    ...
    def get_advantages(self,values, masks, rewards):
        ...
        #returns = returns[::-1] #reverse the array to bring it back to order
        #adv = np.array(returns) - values[:-1]
        adv = returns - values[:-1]
        return returns, tf.keras.utils.normalize(adv)#(adv - np.mean(adv)) / (np.std(adv) + 1e-10)
    ...
    def get_advantages(self, values, masks, rewards):
        ...
        gae = 0
        for i in reversed(range(len(rewards))):
            gae = rewards[i] + self.gamma * values[i + 1] * masks[i]
            returns.insert(0, gae)
        returns = np.array(returns)
        #print ('returns : ',returns)
        #returns = returns[::-1]  # reverse the array to bring it back to order
        #print('reversed returns : ', returns)
        # adv = np.array(returns) - values[:-1]
        adv = returns - values[:-1]
        #print('advantages :', (adv - np.mean(adv)) / (np.std(adv) + 1e-10))
...
def main():
    ...
    file1.writelines('\n************* TIME START:'+str(time.time()/60)+'*************\n')
    print(env.action_space, 'action_space', env.observation_space, 'observation_space\n')
    file1.writelines('{} action_space {} observation_space'.format(env.action_space,env.observation_space))
    ppo_agent.model_actor = ppo_agent.get_model_actor_image(input_dims=(80,80,4), output_dims=n_actions)
    ppo_agent.model_critic = ppo_agent.get_model_critic_image(input_dims=(80,80,4))
    ...
    while not target_reached and iters < Trials:
        ...
        cur_state = np.stack(ppo_agent.seq_memory , axis=2)
        #cur_state = cur_state.reshape((1,)+cur_state.shape)
        ...
        for itr in range(30):
            ...
            #cur_state = cur_state.reshape((1,) + cur_state.shape)
        itr=0
        while not done: #for itr in range(ppo_steps-30):
            ...
            if done:
                env.reset()
                for nesteditr in range(30):
                    action = 0
                    new_state, reward, done, _ = env.step(action)
                    print("Resetting at Trial:{} , ITR:{} -> Action: {}".format(iters, itr+30 , action))
                    file1.writelines("Resetting at Trial:{} , ITR:{} -> Action: {}\n".format(iters, itr+30 , action))
                    new_state = ppo_agent.preprocess(new_state)
                    ppo_agent.seq_memory.append(new_state)
                    # new_state = np.asarray(ppo_agent.seq_memory)
                    # cur_state = new_state
        ...
        print('State shape:' + str(states.shape) + ' actions probs:'+str(actions_probs.reshape(-1, n_actions).shape)+ ' advantages:'+str(advantages.shape)+
        ...
        file1.writelines('State shape:' + str(states.shape) + ' actions probs:'+str(actions_probs.reshape(-1, n_actions).shape)+ ' advantages:'+str(advantages.shape)+
              ' rewards:'+str(np.reshape(rewards, newshape=(-1, 1, 1)).shape)+ ' values:'+str(values[:-1].shape )+ ' actions one hot: '
              + str((np.reshape(actions_onehot, newshape=(-1, n_actions))).shape)+ ' returns: '
              + str(np.reshape(returns, newshape=(-1, 1)).shape)+'\n')
        ...
        actor_loss = ppo_agent.model_actor.fit(
            [states, actions_probs.reshape(-1, n_actions), advantages, np.reshape(rewards, newshape=(-1, 1, 1)), values[:-1]],
            [(np.reshape(actions_onehot, newshape=(-1, n_actions)))], verbose=True, shuffle=True, epochs=8, batch_size=64,
            callbacks=[tensor_board])
        print("training critic")
        file1.writelines("training critic\n")
        critic_loss = ppo_agent.model_critic.fit([states], [np.reshape(returns, newshape=(-1, 1))], epochs=8,
                                                 batch_size=64,verbose=True, shuffle=True, callbacks=[tensor_board])
        ...
        if best_reward > 19 or iters > Trials:
            target_reached = True
        iters += 1
        env.reset()
